<!DOCTYPE html>

<html lang="en">
    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <title>Statistiques</title>
        
        <link rel="stylesheet" href="css/header.css">
        <link rel="stylesheet" href="">
        <link rel=stylesheet href="css/navbar.css">
        <link rel="stylesheet" href="css/responsive.css">
        <link rel="stylesheet" href="css/bootstrap.min.css">
        <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.4.1/css/all.css" integrity="sha384-5sAR7xN1Nv6T6+dT2mhtzEpVJvfS3NScPQTrOxhwjIuvcA67KV2R5Jz6kr4abQsz" crossorigin="anonymous">
        
        <script src="js/bootstrap.min.js"></script>
    </head>
    
    <body>
        <div class="container-fluid" id="mainBox">
            <div class="container-fluid header" id="headerBox">
                <div id="titleBox">
                    <h1>Statistiques</h1>
                </div>
                <div id="line"></div>
            </div>
            
            <div class="container-fluid footer" id="footerBox">
                <div class="container" id="homeBox">
                    <button id="homeBtn" onclick="window.location.href='index.php'">
                        <a href="index.php">
                            <i class="far fa-list-alt fa-2x" style="color: rgba(179, 179, 179, 0.9)"></i>
                        </a>
                    </button>
                    <button id="homeBtn2" onclick="window.location.href='index.php'">
                        <a href="index.php">
                            <i class="far fa-list-alt fa-lg" style="color: rgba(179, 179, 179, 0.9)"></i>
                        </a>
                    </button>
                    <button id="homeBtn3" onclick="window.location.href='index.php'">
                        <a href="index.php">
                            <i class="far fa-list-alt fa-3x" style="color: rgba(179, 179, 179, 0.9)"></i>
                        </a>
                    </button>
                </div>
                <div class="container" id="statsBox">
                    <button id="statsBtn" onclick="window.location.href='stats.php'">
                        <a href="stats.php">
                            <i class="fas fa-chart-line fa-2x" style="color: rgba(0, 0, 0, 0.9)"></i>
                        </a>
                    </button>
                    <button id="statsBtn2" onclick="window.location.href='stats.php'">
                        <a href="stats.php">
                            <i class="fas fa-chart-line fa-lg" style="color: rgba(0, 0, 0, 0.9)"></i>
                        </a>
                    </button>
                    <button id="statsBtn3" onclick="window.location.href='stats.php'">
                        <a href="stats.php">
                            <i class="fas fa-chart-line fa-3x" style="color: rgba(0, 0, 0, 0.9)"></i>
                        </a>
                    </button>
                </div>
                <div class="container" id="addBox">
                    <button id="addBtn" onclick="window.location.href='add.php'">
                        <a href="add.php">
                            <i class="fas fa-plus fa-2x" style="color: white"></i>
                        </a>
                    </button>
                    <button id="addBtn2" onclick="window.location.href='add.php'">
                        <a href="add.php">
                            <i class="fas fa-plus fa-lg" style="color: white"></i>
                        </a>
                    </button>
                    <button id="addBtn3" onclick="window.location.href='add.php'">
                        <a href="add.php">
                            <i class="fas fa-plus fa-3x" style="color: white"></i>
                        </a>
                    </button>
                </div>
                <div class="container" id="editBox">
                        <button id="editBtn" onclick="window.location.href='edit.php'">
                            <a href="edit.php">
                                <i class="far fa-edit fa-2x" style="color: rgba(179, 179, 179, 0.9)"></i>
                            </a>
                        </button>
                        <button id="editBtn2" onclick="window.location.href='edit.php'">
                            <a href="edit.php">
                                <i class="far fa-edit fa-lg" style="color: rgba(179, 179, 179, 0.9)"></i>
                            </a>
                        </button>
                        <button id="editBtn3" onclick="window.location.href='edit.php'">
                            <a href="edit.php">
                                <i class="far fa-edit fa-3x" style="color: rgba(179, 179, 179, 0.9)"></i>
                            </a>
                        </button>
                </div>
                <div class="container" id="accountBox">
                    <button id="accountBtn" onclick="window.location.href='account.php'">
                        <a href="account.php">
                            <i class="far fa-user fa-2x" style="color: rgba(179, 179, 179, 0.9)"></i>
                        </a>
                    </button>
                    <button id="accountBtn2" onclick="window.location.href='account.php'">
                        <a href="account.php">
                            <i class="far fa-user fa-lg" style="color: rgba(179, 179, 179, 0.9)"></i>
                        </a>
                    </button>
                    <button id="accountBtn3" onclick="window.location.href='account.php'">
                        <a href="account.php">
                            <i class="far fa-user fa-3x" style="color: rgba(179, 179, 179, 0.9)"></i>
                        </a>
                    </button>
                </div>
            </div>
        </div>
    
    
    </body>
</html>