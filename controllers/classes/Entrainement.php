...
class Entrainement
{
    private $tab = array(), $nom, $lieu, $date, $distance, $nbserie, $nbvolees, $nbfleches, $ID_blason, $ID_arc, $ID_user;
    ...
    { //on check si les valeurs saisi sont valides pour etre envoyés dans la base de données.
        ...
        if (!empty($ID_blason) and !empty($ID_arc) and !empty($nom) and !empty($lieu) and !empty($date)
            ...
            if (is_numeric($ID_blason) and is_numeric($ID_arc) and strlen($nom) <= 30 and strlen($lieu) <= 200
                ...
                $stmt = $con->query("SELECT ID_USER FROM users WHERE ID_USER='$ID_user'");
    ...
    public function creerEntrainement($nominput, $lieuinput, $date, $distance, $ID_arc, $ID_blason, $nbserie, $nbvolees, $nbfleches, $ID_user)
    {
        ...
        if ($this->checkEnt($nom, $lieu, $date, $distance, $ID_arc, $ID_blason, $nbserie, $nbvolees, $nbfleches, $ID_user) == true) {
            ...
            for ($i = 0; $i < $nbserie; $i++) {
                ...
                $requ4 = $con->exec("INSERT INTO `serie` (`ID_SERIE`, `ID_ENT_USER`, `PTSSERIE`, `NBRVOLSERIE`, `MOYSERIE`, `PCTDIXSERIE`, `PCTHUITSERIE`, `ID_ENT`) 
                  VALUES (NULL,'$new_ID_Ent', NULL, '$nbvolees', NULL, NULL, NULL, '$ID_ENT') ") or die(print_r($bdd->errorInfo()));


                $requ5 = $con->query("SELECT ID_SERIE FROM serie WHERE ID_ENT_USER='$new_ID_Ent' and NBRVOLSERIE='$nbvolees' and ID_ENT='$ID_ENT'") or die(print_r($bdd->errorInfo()));
                ...
                for ($j = 0; $j < $nbvolees; $j++) {
                    ...
                    $requ6 = $con->exec("INSERT INTO `volee` (`ID_VOL`, `ID_SERIE`, `PTSVOL`, `NBRFLECHEVOL`, `MOYVOL`, `PCTDIXVOL`, `PCTHUITVOL`) 
                              VALUES (NULL, '$ID_SERIE', NULL, '$nbfleches', NULL, NULL, NULL)")or die(print_r($bdd->errorInfo()));

                    $requ7 = $con->query("SELECT ID_VOL FROM volee WHERE ID_SERIE='$ID_SERIE' and NBRFLECHEVOL='$nbfleches'") or die(print_r($bdd->errorInfo()));
                    ...
                    for ($z = 0; $z < $nbfleches; $z++) {
                        $requ7 = $con->exec("INSERT INTO `fleche` (`ID_FLECHE`, `ID_VOL`, `PTSFLECHE`) 
